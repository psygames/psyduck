...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public PersistentTokenRepository persistentTokenRepository() {
		MongoTokenRepositoryImpl tokenRepository = new MongoTokenRepositoryImpl();
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				...
				.antMatchers("/getVerifyCode", "/login", "/register", "/users/add", "/signout", "/login/invalid")
				...
				.authenticationDetailsSource(customAuthenticationDetailsSource).and()
				// 2.自定义验证码过滤器，在springsecurity执行密码验证过滤之前，先执行我们的验证码验证
				.addFilterBefore(new VerifyFilter(), UsernamePasswordAuthenticationFilter.class)

				// 退出登录
				.logout().logoutUrl("/signout").deleteCookies("JSESSIONID").logoutSuccessHandler(logoutSuccessHandler)
				.permitAll()
				...
				// 当达到最大值时，旧用户被踢出后的操作
				.expiredSessionStrategy(new CustomExpiredSessionStrategy()).sessionRegistry(sessionRegistry());// 主动踢人
